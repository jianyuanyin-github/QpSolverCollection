...
void QpCoeff::setup(int dim_var, int dim_eq, int dim_ineq)
{
  ...
  ineq_vec_lb_.setZero(dim_ineq);  // for two-sided constraints
  ineq_vec_ub_.setZero(dim_ineq);
...
{
  QSC_INFO_STREAM(
    header << "dim_var: " << dim_var_ << ", dim_eq: " << dim_eq_ << ", dim_ineq: " << dim_ineq_);
}
void QpCoeff::dump(std::ofstream & ofs) const
{
  ...
  ofs << "ineq_vec_lb (for two-sided constraints):\n" << ineq_vec_lb_.transpose() << std::endl;
  ofs << "ineq_vec_ub (for two-sided constraints):\n" << ineq_vec_ub_.transpose() << std::endl;
...
Eigen::VectorXd QpSolver::solve_bothside(QpCoeff & qp_coeff)
{
  return solve(
    qp_coeff.dim_var_, qp_coeff.dim_eq_, qp_coeff.dim_ineq_, qp_coeff.obj_mat_, qp_coeff.obj_vec_,
    qp_coeff.eq_mat_, qp_coeff.eq_vec_, qp_coeff.ineq_mat_, qp_coeff.ineq_vec_lb_,
    qp_coeff.ineq_vec_ub_, qp_coeff.x_min_, qp_coeff.x_max_);
}
...
bool QpSolverCollection::isQpSolverEnabled(const QpSolverType & qp_solver_type)
{
  if (qp_solver_type == QpSolverType::Any) {
    ...
    QSC_ERROR_STREAM(
      "[isQpSolverEnabled] Unsupported QP solver: "
      << std::to_string(static_cast<int>(qp_solver_type)));
    return false;
  }
}
...
std::shared_ptr<QpSolver> QpSolverCollection::allocateQpSolver(const QpSolverType & qp_solver_type)
{
  ...
  if (qp_solver_type == QpSolverType::Any) {
    ...
    QSC_ERROR_STREAM(
      "[allocateQpSolver] Unsupported QP solver: "
      << std::to_string(static_cast<int>(qp_solver_type)));
  }

  if (!qp) {
    QSC_ERROR_STREAM(
      "[allocateQpSolver] Failed to initialize QP solver: " << std::to_string(qp_solver_type));
  }
